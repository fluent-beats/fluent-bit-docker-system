 ...
 */
static void dsystem_unix_socket_write(struct flb_in_dsystem_config *ctx)
{
    char request[30];

    strcpy(request, "GET /info HTTP/1.0\r\n\r\n");
 ...
 * @param in_context    void Pointer used to cast to flb_in_dsystem_config
 *
 * @return int Always returns success
 */
static int dsystem_unix_socket_read(struct flb_input_instance *ins,
                                   struct flb_config *config, void *in_context)
{
    ...
    char *body;
    size_t str_len = 0;
    struct flb_in_dsystem_config *ctx = in_context;
 ...
 * @return int 0 on success, -1 on failure
 */
static int dsystem_unix_socket_connect(struct flb_in_dsystem_config *ctx)
{
    unsigned long len;
 ...
 *                      flb_in_dsystem_config
 *
 * @return int Always returns success
 */
static int cb_dsystem_collect(struct flb_input_instance *ins,
                             struct flb_config *config, void *in_context)
{
    struct flb_in_dsystem_config *ctx = in_context;
    if(dsystem_unix_socket_connect(ctx) != -1)
    {
        dsystem_unix_socket_write(ctx);
        dsystem_unix_socket_read(ins, config, in_context);
    }
 ...
 * @return int 0 on success, -1 on failure
 */
static int in_dsystem_init(struct flb_input_instance *ins,
                          struct flb_config *config, void *data)
{
    struct flb_in_dsystem_config *ctx = NULL;
    (void) data;

    /* Allocate space for the configuration */
    ctx = dsystem_config_init(ins, config);
    if (!ctx) {
        return -1;
    }
    ctx->ins = ins;
    ...
    ctx->coll_id = flb_input_set_collector_time(ins, cb_dsystem_collect,
                                                ctx->collect_interval,
                                                0, config);
    if(ctx->coll_id < 0){
        flb_plg_error(ctx->ins, "could not set collector for IN_DOCKER_SYSTEM plugin");
        dsystem_config_destroy(ctx);
        return -1;
    }
 ...
 *
 * @param data    Pointer cast to flb_in_dsystem_config
 * @param config  Unused
 *
 * @return int    Always returns 0
 */
static int in_dsystem_exit(void *data, struct flb_config *config)
{
    (void) config;
    struct flb_in_dsystem_config *ctx = data;
...
static struct flb_config_map config_map[] = {
    {
     ...
     0, FLB_TRUE, offsetof(struct flb_in_dsystem_config, collect_interval),
     "Collect interval."
    },
    {
     FLB_CONFIG_MAP_STR, "unix_path", DEFAULT_UNIX_SOCKET_PATH,
     0, FLB_TRUE, offsetof(struct flb_in_dsystem_config, unix_path),
     "Define Docker unix socket path to read events"
    },
    {
     FLB_CONFIG_MAP_SIZE, "buffer_size", DEFAULT_BUF_SIZE,
     0, FLB_TRUE, offsetof(struct flb_in_dsystem_config, buf_size),
    ...
    },
    {
     ...
     0, FLB_TRUE, offsetof(struct flb_in_dsystem_config, key),
...
/* Plugin reference */
struct flb_input_plugin in_docker_system_plugin = {
    .name         = "docker_system",
    .description  = "Docker System input plugin",
    .cb_init      = in_dsystem_init,
    .cb_pre_run   = NULL,
    .cb_collect   = cb_dsystem_collect,
    .cb_flush_buf = NULL,
    .cb_exit      = in_dsystem_exit,
